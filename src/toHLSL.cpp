...
GLLang ChooseLanguageHLSL(Shader* psShader)
{
...
const char* GetVersionStringHLSL(GLLang language)
{
    switch (language)
    {
        case LANG_ES_100:
...
GLLang ToHLSL::SetLanguage(GLLang suggestedLanguage)
{
    language = suggestedLanguage;
    if (language == LANG_DEFAULT)
    {
        language = ChooseLanguageHLSL(psContext->psShader);
    }
    return language;
}

// Go through all declarations and remove reserve UAV occupied binding points
void ResolveStructuredBufferBindingSlotsHLSL(ShaderPhase *psPhase, HLSLCrossCompilerContext *psContext, GLSLCrossDependencyData *glslDependencyData)
{
...
static std::string GetOutputStructNameForType(SHADER_TYPE type)
{
    switch (type)
    {
    case VERTEX_SHADER:
    ...
    case HULL_SHADER:
        /*if (psContext->psShader->asPhases[psContext->currentPhase].ePhase == HS_FORK_PHASE ||
            ...
            return "PatchConstant";*/ //TODO(pema): Figure out what this does
        return "ControlPoint";
    case DOMAIN_SHADER:
...
static std::string GetInputStructNameForType(SHADER_TYPE type)
{
    switch (type)
    {
    case VERTEX_SHADER:
...
{
    return GetOutputStructNameForType(psContext->psShader->eShaderType);
}

std::string ToHLSL::GetInputStructName() const
{
    return GetInputStructNameForType(psContext->psShader->eShaderType);
}
...
bool ToHLSL::Translate()
{
    ...
    bstring extensions = bfromcstralloc(1024 * 10, GetVersionStringHLSL(language));
    ...
    for (ui32Phase = 0; ui32Phase < psShader->asPhases.size(); ui32Phase++)
    {
        ...
        ResolveStructuredBufferBindingSlotsHLSL(&phase, psContext, psContext->psDependencies);
...
bool ToHLSL::DeclareExtraFunction(const std::string &name, bstring body)
{
...
void ToHLSL::UseExtraFunctionDependency(const std::string &name)
{
    if (m_FunctionDefinitions.find(name) != m_FunctionDefinitions.end())
        return;
