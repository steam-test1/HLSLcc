...
void ToHLSL::DeclareConstBufferShaderVariable(const char* varName, const struct ShaderVarType* psType, const struct ConstantBuffer* psCBuf, int unsizedArray, bool addUniformPrefix, bool reportInReflection)
{
...
void ToHLSL::PreDeclareStructType(const std::string &name, const struct ShaderVarType* psType)
{
    bstring glsl = *psContext->currentGLSLString;
    uint32_t i;
...
bool ToHLSL::RenderTargetDeclared(uint32_t input)
{
...
void ToHLSL::AddBuiltinInput(const Declaration* psDecl, const char* builtinName)
{
    Shader* psShader = psContext->psShader;
...
void ToHLSL::AddBuiltinOutput(const Declaration* psDecl, int arrayElements, const char* builtinName)
{
    bstring glsl = *psContext->currentGLSLString;
    Shader* psShader = psContext->psShader;
...
void ToHLSL::HandleOutputRedirect(const Declaration *psDecl, const char *Precision)
{
    const Operand *psOperand = &psDecl->asOperands[0];
    Shader *psShader = psContext->psShader;
    bstring glsl = *psContext->currentGLSLString;
...
void ToHLSL::AddUserOutput(const Declaration* psDecl)
{
    bstring glsl = *psContext->currentGLSLString;
    Shader* psShader = psContext->psShader;
...
void ToHLSL::ReportStruct(const std::string &name, const struct ShaderVarType* psType)
{
...
void ToHLSL::DeclareUBOConstants(const uint32_t ui32BindingPoint, const ConstantBuffer* psCBuf, bstring glsl)
{
    uint32_t i;
    ...
    {
        std::string instanceName = UniformBufferInstanceNameHLSL(psContext, psCBuf->name);
        bformata(glsl, "} %s;\n", instanceName.c_str());
    }
    else
        bcatcstr(glsl, "};\n");
...
bool DeclareRWStructuredBufferTemplateTypeAsIntegerHLSL(HLSLCrossCompilerContext* psContext, const Operand* psOperand)
{
    ...
    const uint32_t isRaw, const uint32_t isUAV, const uint32_t hasEmbeddedCounter, const uint32_t stride, bstring glsl)
{
    ...
    if (!isRaw)
    {
        ...
        if (isUAV && DeclareRWStructuredBufferTemplateTypeAsIntegerHLSL(psContext, psOperand))
            typeStr = "int";
...
void ToHLSL::HandleInputRedirect(const Declaration *psDecl, const char *Precision)
{
...
void ToHLSL::TranslateDeclaration(const Declaration* psDecl)
{
    bstring glsl = *psContext->currentGLSLString;
    Shader* psShader = psContext->psShader;
...
//                  bformata(glsl, "out float4 %s;\n", psDecl->asOperands[0].specialName.c_str());
        ...
        case OPCODE_DCL_OUTPUT_SIV:
        {
            ...
            {
                int dummy = 0;
                inputName = psContext->GetDeclaredOutputName(&psDecl->asOperands[0], &dummy, NULL, NULL, 1);
            }
            else
                inputName = psContext->GetDeclaredInputName(psOperand, NULL, 1, NULL);
            ...
            if (IsSpecialSystemValueInput(psOperand->eSpecialName))
                structName = "";
            else if (isOut)
                structName = GetOutputStructName();
            else
...
// TODO(pema): Pass these through
bool ToHLSL::TranslateSystemValue(const Operand *psOperand, const ShaderInfo::InOutSignature *sig, std::string &result, uint32_t *pui32IgnoreSwizzle, bool isIndexed, bool isInput, bool *outSkipPrefix, int *iIgnoreRedirect)
{
    ASSERT(sig != NULL);
