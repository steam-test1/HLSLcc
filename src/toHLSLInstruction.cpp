...
void ToHLSL::AddOpAssignToDestWithMask(const Operand* psDest,
...
void ToHLSL::AddAssignToDest(const Operand* psDest,
...
void ToHLSL::AddAssignPrologue(int numParenthesis, bool isEmbedded /* = false*/)
{
    bstring glsl = *psContext->currentGLSLString;
...
void ToHLSL::AddComparison(Instruction* psInst, ComparisonType eType,
    uint32_t typeFlag)
{
...
void ToHLSL::AddMOVBinaryOp(const Operand *pDest, Operand *pSrc, uint32_t precise, bool isEmbedded /* = false*/)
{
...
void ToHLSL::AddMOVCBinaryOp(const Operand *pDest, const Operand *src0, Operand *src1, Operand *src2, uint32_t precise)
{
    bstring glsl = *psContext->currentGLSLString;
...
void ToHLSL::CallBinaryOp(const char* name, Instruction* psInst,
...
void ToHLSL::CallTernaryOp(const char* op1, const char* op2, Instruction* psInst,
...
void ToHLSL::CallHelper3(const char* name, Instruction* psInst,
...
void ToHLSL::CallHelper2(const char* name, Instruction* psInst,
...
void ToHLSL::CallHelper2Int(const char* name, Instruction* psInst,
    int dest, int src0, int src1, int paramsShouldFollowWriteMask)
{
...
void ToHLSL::CallHelper2UInt(const char* name, Instruction* psInst,
    int dest, int src0, int src1, int paramsShouldFollowWriteMask)
{
...
void ToHLSL::CallHelper1(const char* name, Instruction* psInst,
...
std::string ToHLSL::GetVulkanDummySamplerName()
{
...
void ToHLSL::TranslateTexelFetch(
    Instruction* psInst,
    ...
    bstring glsl)
{
    ...
    TranslateOperandSwizzleWithMaskHLSL(psContext, &psInst->asOperands[2], psInst->asOperands[0].GetAccessMask(), 0);
    AddAssignPrologue(numParenthesis);
}
...
void ToHLSL::GetResInfoData(Instruction* psInst, int index, int destElem)
{
    bstring glsl = *psContext->currentGLSLString;
    int numParenthesis = 0;
...
void ToHLSL::TranslateTextureSample(Instruction* psInst,
    uint32_t ui32Flags)
{
    bstring glsl = *psContext->currentGLSLString;
    int numParenthesis = 0;
    ...
    if (!(ui32Flags & TEXSMP_FLAG_DEPTHCOMPARE) || (ui32Flags & TEXSMP_FLAG_GATHER))
    {
        ...
        TranslateOperandSwizzleWithMaskHLSL(psContext, psSrcTex, psDest->GetAccessMask(), 0);
    }
    AddAssignPrologue(numParenthesis);
}

const char* swizzleStringHLSLHLSL[] = { ".x", ".y", ".z", ".w" };
...
void ToHLSL::TranslateDynamicComponentSelection(const ShaderVarType* psVarType, const Operand* psByteAddr, uint32_t offset, uint32_t mask)
{
    bstring glsl = *psContext->currentGLSLString;
...
void ToHLSL::TranslateShaderStorageStore(Instruction* psInst)
{
    bstring glsl = *psContext->currentGLSLString;
    int component;
    int srcComponent = 0;
...
void ToHLSL::TranslateShaderStorageLoad(Instruction* psInst)
{
    bstring glsl = *psContext->currentGLSLString;
    int component;
    Operand* psDest = 0;
...
void ToHLSL::TranslateAtomicMemOp(Instruction* psInst)
{
    bstring glsl = *psContext->currentGLSLString;
    int numParenthesis = 0;
...
void ToHLSL::TranslateConditional(
    Instruction* psInst,
    bstring glsl)
{
...
void ToHLSL::HandleSwitchTransformation(Instruction* psInst, bstring glsl)
{
...
void ToHLSL::TranslateInstruction(Instruction* psInst, bool isEmbedded /* = false */)
{
    bstring glsl = *psContext->currentGLSLString;
    int numParenthesis = 0;
    ...
    switch (psInst->eOpcode)
    {
        ...
        case OPCODE_LD_UAV_TYPED:
        {
            ...
            bcatcstr(glsl, ")");
            TranslateOperandSwizzleWithMaskHLSL(psContext, psSrc, psDest->ui32CompMask, 0);
            AddAssignPrologue(numParenthesis);
            break;
        }
        case OPCODE_STORE_RAW:
