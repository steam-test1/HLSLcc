...
{
    TranslateOperandSwizzleWithMaskHLSL(psContext, psOperand, OPERAND_4_COMPONENT_MASK_ALL, iRebase);
}
...
{
    TranslateOperandSwizzleWithMaskHLSL(*psContext->currentGLSLString, psContext, psOperand, ui32ComponentMask, iRebase);
}
...
void ToHLSL::TranslateOperandIndex(const Operand* psOperand, int index)
{
    int i = index;
...
void ToHLSL::TranslateOperandIndexMAD(const Operand* psOperand, int index, uint32_t multiply, uint32_t add)
{
    int i = index;
...
void ToHLSL::TranslateVariableNameWithMask(const Operand* psOperand, uint32_t ui32TOFlag, uint32_t* pui32IgnoreSwizzle, uint32_t ui32CompMask, int *piRebase, bool forceNoConversion)
{
...
void ToHLSL::DeclareDynamicIndexWrapper(const struct ShaderVarType* psType)
{
    DeclareDynamicIndexWrapper(psType->name.c_str(), psType->Class, psType->Type, psType->Rows, psType->Columns, psType->Elements);
}

void ToHLSL::DeclareDynamicIndexWrapper(const char* psName, SHADER_VARIABLE_CLASS eClass, SHADER_VARIABLE_TYPE eType, uint32_t ui32Rows, uint32_t ui32Columns, uint32_t ui32Elements)
{
...
void ToHLSL::TranslateVariableNameWithMask(bstring glsl, const Operand* psOperand, uint32_t ui32TOFlag, uint32_t* pui32IgnoreSwizzle, uint32_t ui32CompMask, int *piRebase, bool forceNoConversion)
{
...
//          ASSERT(numComponents == 1);
    ...
    {
        TranslateOperandSwizzleWithMaskHLSL(glsl, psContext, psOperand, ui32CompMask, piRebase ? *piRebase : 0);
        *pui32IgnoreSwizzle = 1;
    }
...
void ToHLSL::TranslateOperand(const Operand* psOperand, uint32_t ui32TOFlag, uint32_t ui32ComponentMask, bool forceNoConversion)
{
    TranslateOperand(*psContext->currentGLSLString, psOperand, ui32TOFlag, ui32ComponentMask, forceNoConversion);
}

void ToHLSL::TranslateOperand(bstring glsl, const Operand* psOperand, uint32_t ui32TOFlag, uint32_t ui32ComponentMask, bool forceNoConversion)
{
    ...
    {
        TranslateOperandSwizzleWithMaskHLSL(glsl, psContext, psOperand, ui32ComponentMask, iRebase);
    }
